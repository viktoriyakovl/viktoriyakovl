...
| Параметр | Значение |
| :--- | :--- |
| **Задача** | [Backend] Подход к хранению медиаконтента (TPPDEV-2125) |
| **Статус** | `IN REVIEW` (CLOSED) |
| **Feature Owner** | *[Не указано]* |
...
#### Таблица `media_files` (примерное название сущности)
...
Для интеграции необходимо написать **shared-библиотеку**, которая будет ходить в микросервис `MediaContent` (аналогично `Localizator`).

Связь контента (товара/новости) с медиафайлами осуществляется через хранение идентификаторов в БД микросервисов контента (`Webshop`, `NewsV2`, `ContentForge`, `GameInfo`).

### Пример интеграции (NewsV2)
